...
/**
 * Renders an individual line of content based on its Markdown structure.
 */
const ContentBlock = ({ content }) => {
    let element;
    let text = content.trim();

    // Function to replace **text** with <strong>text</strong> using Tailwind styling
    const processText = (line) => {
        // Regex to find **text** and replace it with strong tags with Tailwind class
        return line
            .replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-orange-600">$1</strong>')
            // Handle single * for 'B', 'R', 'A', 'T' in the source text
            .replace(/\*(.*?)\*/g, '<strong class="font-semibold text-orange-600">$1</strong>');
    };
    ...
    // 1. Check for H3 (###)
    if (text.startsWith('###')) {
        element = <h3 className="text-xl font-extrabold text-teal-700 border-l-4 border-teal-400 pl-4 mt-6 mb-3">
            {text.replace('###', '').trim()}
        </h3>;
    } 
    // 2. Check for H4 (####)
    else if (text.startsWith('####')) {
        element = <h4 className="text-lg font-bold text-gray-800 border-b border-gray-200 pb-1 mt-4 mb-2">
            {processText(text.replace('####', '').trim())}
        </h4>;
    } 
    // 3. Check for List Items (*) - handled in the parent loop below
    else if (text.startsWith('*')) {
        const processedText = processText(text.replace(/^\*\s*/, ''));
        element = <li className="text-gray-600 mb-1 leading-relaxed" dangerouslySetInnerHTML={{ __html: processedText }} />;
    } 
    // 4. Check for Separator (---)
    else if (text.startsWith('---')) {
        element = <hr className="border-t border-gray-300 my-4" />;
    } 
    // 5. Default Paragraph
    else {
        const processedText = processText(text);
        element = <p className="text-gray-700 leading-relaxed mb-3" dangerouslySetInnerHTML={{ __html: processedText }} />;
    }
...
/**
 * Parses and renders the raw text using the ContentBlock component.
 */
const StructuredContentRenderer = ({ rawText }) => {
    if (!rawText) return <p className="text-gray-500 italic">No detailed recommendations provided yet.</p>;

    // Split the raw text by newline characters and filter out empty lines
    const lines = rawText.split('\n').filter(line => line.trim() !== '');

    // Group the lines into logical blocks (paragraphs, lists, or headers)
    const blocks = [];
    let currentBlock = [];
    ...
    lines.forEach((line, index) => {
        ...
        if (trimmedLine.startsWith('###') || trimmedLine.startsWith('####') || trimmedLine.startsWith('---')) {
            ...
            // Start a new block for the header/separator
            blocks.push([trimmedLine]);
            currentBlock = [];
        } else if (trimmedLine.startsWith('*')) {
            // Check if the previous line was NOT a list item. If so, push current block and start new list block.
            ...
            // General text (paragraph or bolded line)
            currentBlock.push(trimmedLine);
        }
    ...
    return (
        <div className="space-y-4">
            {blocks.map((block, blockIndex) => {
                // Check if the block is a list group (starts with a list item)
                const isList = block.some(line => line.startsWith('*'));

                if (isList) {
                    // Separate non-list lines (like titles or paragraphs right above the list)
                    const precedingText = block.filter(line => !line.startsWith('*'));
                    const listItems = block.filter(line => line.startsWith('*'));
                    
                    return (
                        <div key={blockIndex} className="content-block list-group">
                            {precedingText.map((line, i) => (
                                // Render preceding text blocks (e.g., "1. Diet Habits:")
                                <ContentBlock key={`${blockIndex}-pre-${i}`} content={line} />
                            ))}
                            <ul className="list-disc pl-5 space-y-2">
                                {listItems.map((line, i) => (
                                    <ContentBlock key={`${blockIndex}-li-${i}`} content={line} />
                                ))}
                            </ul>
                        </div>
                    );
                ...
                // Otherwise, render as standard paragraphs/headers
                return (
                    <div key={blockIndex} className="content-block">
                        {block.map((line, i) => (
                            <ContentBlock key={`${blockIndex}-${i}`} content={line} />
                        ))}
                    </div>
                );
            })}
  ...
  return (
    <div className="p-4 sm:p-8 bg-white rounded-xl shadow-2xl w-full max-w-3xl mx-auto">
      ...
      <div className="p-6 border border-blue-200 rounded-lg bg-blue-50 shadow-inner mb-6">
        ...
        <h3 className="text-xl font-extrabold text-blue-700 mb-4 border-b border-blue-300 pb-2">
          Lifestyle & Diet Recommendations
        </h3>
        <StructuredContentRenderer rawText={diagnosis.gemini} />
